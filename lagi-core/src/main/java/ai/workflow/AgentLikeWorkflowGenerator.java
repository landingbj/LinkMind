...
public class AgentLikeWorkflowGenerator {
    ...
    private static void stage2_TaskAnalysis(GenerationContext context) {
        ...
        String analysis = result.getChoices().get(0).getMessage().getContent();
        
        context.taskAnalysis = analysis;
        log.info("Task analysis result: {}", analysis);
    }
    ...
    private static String buildTaskAnalysisPrompt() {
        return "# 任务分析\n\n" +
               ...
               "[一句话描述用户要实现的目标]\n\n" +
               "### 核心功能识别\n" +
               "[识别是否有直接匹配的节点类型，如：文生视频 → text2video]\n\n" +
               "### 输入信息\n" +
               "- [输入1]\n\n" +
               "### 输出结果\n" +
               "- [输出1]\n\n" +
               ...
               "[简单/中等/复杂]，理由：[...]\n\n" +
               "### 简洁性检查\n" +
               "[说明为什么这是最简洁的方案，有没有可以合并的步骤]\n" +
               "```\n\n" +
               "## 示例\n\n" +
               "❌ **错误示例**（过度拆分）：\n" +
               "用户需求：生成一个文生视频的工作流\n" +
               "错误分析：1. 用LLM理解文本 → 2. 生成图片 → 3. 图片转视频\n" +
               "问题：text2video节点可以直接完成，不需要拆分\n\n" +
               "✅ **正确示例**（简洁）：\n" +
               "用户需求：生成一个文生视频的工作流\n" +
               "正确分析：1. 使用text2video节点直接从文本生成视频\n\n" +
               "请只输出分析结果，不要包含其他说明。";
    }

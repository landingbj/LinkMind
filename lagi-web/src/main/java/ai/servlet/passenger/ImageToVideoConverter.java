...
public class ImageToVideoConverter {
    ...
    public static File convertImagesToVideo(List<String> imageUrls, String outputDir, double frameRate) throws IOException {
        logger.info("[FFmpeg转换] 开始转换图片为视频，图片数量: {}, 输出目录: {}, 帧率: {}", imageUrls.size(), outputDir, frameRate);
    ...
    private static void downloadImage(String imageUrl, File outputFile) throws IOException {
        try (java.io.InputStream in = new java.net.URL(imageUrl).openStream();
            ...
            logger.error("[FFmpeg转换] 下载图片失败: url={}, file={}, error={}", imageUrl, outputFile.getAbsolutePath(), e.getMessage(), e);
            throw e;
        }
    }

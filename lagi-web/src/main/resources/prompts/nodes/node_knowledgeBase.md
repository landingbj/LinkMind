...
| data.inputsValues.query        | object | 是  | 检索关键词：<br>`type`："constant"（固定字符串"知识库查询" ）或"ref"<br>` （如["start_0","query"])                                                                                                                                |
| data.outputs.properties.result | string | 是  | 知识库检索结果：  `type`: string                                                                                                                                                                                    |

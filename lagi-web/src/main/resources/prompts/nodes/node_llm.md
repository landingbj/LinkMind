...
| data.inputs              | object | 是  | 输入的属性定义,该节点为固定的结构，不可修改, object : { "type": "object", "required": ["model", "prompt"], // 必须包含这两个参数 "properties": { "model": { "type": "string" }, "prompt": { "type": "string", "extra": { "formComponent": "prompt-editor" } // 固定extra字段 }                                     |
...
| data.inputsValues.prompt | object | 是  | 提示词：<br>`type`："template"（模板，含{{变量}}）<br>`content`：提示词内容（如"以{{start_0.query}}为题写文章"）                                                                                                                                                                                             |
| data.outputs             | object | 是  | 输出属性的定义,该节点为固定的结构，不可修改, object : { "type": "object", "properties": { "result": { "type": "string" } } }                                                                                                                                                                          |

...
2. **使用标准节点字段名**：如果已提供节点输出字段规范，在描述步骤的输出信息时，必须使用标准字段名：
   - llm、knowledge-base、program 等大多数节点的输出字段为 `result`
   - intent-recognition 节点的输出字段为 `intent`
   - api 节点的输出字段为 `statusCode` 和 `body`
   - 禁止使用 answer、response、output 等自定义字段名
3. **禁止描述额外的输入字段**：在描述步骤的输入信息时，只能提及节点定义中存在的字段：
   - 例如 TTS 节点只能描述：text、model、voice、volume、format
   - 禁止描述：voiceModel、languageCode、speed、pitch 等未定义的字段
   - 例如 LLM 节点只能描述：model、prompt
   - 例如 Knowledge-Base 节点只能描述：category、query
4. 必须明确标识流程的开始和结束
5. 如果有条件判断，要清楚说明每个分支的条件和执行路径
6. 如果有循环，要说明循环的数据来源、循环体的操作、循环结束条件
7. 要注意数据依赖关系，确保每个步骤的输入在前序步骤中已经产生
8. 保持描述清晰、结构化，便于后续的节点匹配和JSON生成
9. **开始步骤的输入**和**结束步骤的输出**要与已提取的变量保持一致
10. **识别并拆分相同类型的多个步骤**：当用户描述中包含多个相同功能但配置不同的步骤时（如"先总结后生成"需要2个LLM），必须明确拆分为独立的步骤

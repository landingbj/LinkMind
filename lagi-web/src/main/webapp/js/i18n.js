;(function () {
    ...
    const textMap = {
        ...
        '查看服务端持久化的 LLM Token 用量与趋势（与聊天会话字符估算无关）。': 'View persisted LLM token usage from the server (not local character estimates).',
        '全部': 'All',
        '说明：费用为估算值（默认按 0.002 RMB / 1K Tokens）。自定义日期时按跨度映射为 today / 7d / 30d / all。': 'Note: Cost is estimated (0.002 RMB / 1K Tokens). Custom dates map to the nearest API window (today / 7d / 30d / all).',
        '记录条数': 'Records',
        '用量明细': 'Usage Details',
        'Prompt': 'Prompt',
        ...
        '全部类型': 'All Types',
        '敏感词': 'Sensitive',
        '输入敏感词': 'Input Sensitive',
        '输出敏感词': 'Output Sensitive',
        '优先级': 'Priority',
        '优先词': 'Priority Word',
        '停止词': 'Stopping',
        '继续词': 'Continue',
        '拦截或处理用户问题，命中 block 时会直接阻止请求。': 'Filters user prompts. A block match stops the request immediately.',
        '过滤模型回复内容，支持 block、mask、erase。': 'Filters model responses with block, mask, or erase.',
        '影响候选答案或检索结果排序，不会拦截请求。': 'Affects answer or retrieval ranking; it does not block requests.',
        '用于判断新话题和会话边界，命中后不再强行续接上一轮上下文，不会拦截请求。': 'Marks new topics or conversation boundaries. A match avoids forcing continuation from the previous context; it does not block requests.',
        '用于判断用户想继续上一轮话题，会影响上下文拼接和检索。': 'Marks continuation intent and affects context stitching and retrieval.',
        '在分组里配置规则和级别：1=block，2=mask，3=erase。': 'Configure rules and level in Groups: 1=block, 2=mask, 3=erase.',
        '在分组里配置规则和级别：1=block，2=mask，3=erase。多条规则支持英文逗号、中文逗号、顿号、分号或换行分隔。': 'Configure rules and level in Groups: 1=block, 2=mask, 3=erase. Multiple rules can be separated by English commas, Chinese commas, enumeration commas, semicolons, or line breaks.',
        '在规则中填写优先匹配词，多个规则用逗号分隔。': 'Enter priority terms in Rules, separated by commas.',
        '在规则中填写优先匹配词，支持英文逗号、中文逗号、顿号、分号或换行分隔。': 'Enter priority terms in Rules. Use English commas, Chinese commas, enumeration commas, semicolons, or line breaks as separators.',
        '在规则中填写表示话题结束或新话题开始的词，多个规则用逗号分隔。': 'Enter terms that indicate a topic ending or new topic, separated by commas.',
        '在规则中填写表示话题结束或新话题开始的词，支持英文逗号、中文逗号、顿号、分号或换行分隔。': 'Enter terms that indicate a topic ending or new topic. Use English commas, Chinese commas, enumeration commas, semicolons, or line breaks as separators.',
        '在规则中填写继续追问类词，多个规则用逗号分隔。': 'Enter continuation terms, separated by commas.',
        '在规则中填写继续追问类词，支持英文逗号、中文逗号、顿号、分号或换行分隔。': 'Enter continuation terms. Use English commas, Chinese commas, enumeration commas, semicolons, or line breaks as separators.',
        '过滤器类型': 'Filter Type',
        '请选择过滤器类型': 'Please choose filter type',
        '注意：只能选择以上系统支持的过滤器类型，自定义名称不会生效': 'Note: Only the system-supported filter types are valid. Custom names are ignored.',
        ...
        '规则 (支持逗号、顿号、分号或换行分隔):': 'Rules (comma, enumeration comma, semicolon, or line break separated):',
        '例如: car,weather,社*保&#10;多个规则用逗号分隔，支持正则表达式': 'e.g. car,weather,soc*ial&#10;Separate multiple rules by comma; regex is supported.',
        '例如: car,weather,社*保&#10;支持英文逗号、中文逗号、顿号、分号或换行分隔，支持正则表达式': 'e.g. car,weather,soc*ial&#10;Separate rules with English commas, Chinese commas, enumeration commas, semicolons, or line breaks; regex is supported.',
        ...
        '提示：多个规则用逗号分隔，支持正则表达式。如果是敏感词过滤器，请在"分组"中配置级别和规则。': 'Tip: Separate multiple rules with commas; regex is supported. For sensitive filters, configure level and rules in "Groups".',
        '提示：支持英文逗号、中文逗号、顿号、分号或换行分隔，支持正则表达式。如果是敏感词过滤器，请在“分组”中配置级别和规则。': 'Tip: Use English commas, Chinese commas, enumeration commas, semicolons, or line breaks as separators; regex is supported. For sensitive filters, configure level and rules in "Groups".',
        ...
        '输出速度': 'Output Speed',
        '用于查看平台请求活动概览（Spend / Requests / Tokens）。': 'Used to view platform activity overview (Spend / Requests / Tokens).',
        '点击后可查看活动指标与趋势。': 'Click to view activity metrics and trends.',
        '用于查看请求级日志列表与筛选。': 'Used to view request-level logs and filters.',
        '点击后可查看日志明细。': 'Click to view log details.',
        '额度中心': 'Credits',
        '用于查看账户额度、充值与最近交易记录。': 'Used to view account balance, top-up and recent transactions.',
        '点击后可进行额度管理。': 'Click to manage credits.',
        '个人账户: 未登录': 'Personal account: Not signed in',
        '充值额度': 'Top Up Credits',
        '立即充值': 'Top Up Now',
        '自动充值': 'Auto Top-up',
        '当余额低于阈值时自动发起充值。<br/>你可以在此页面随时开关此能力。': 'When balance falls below threshold, top-up starts automatically.<br/>You can toggle this feature on this page anytime.',
        '启用': 'Enable',
        '禁用': 'Disable',
        '查看消费记录': 'View Usage Records',
        ...
        '成功': 'Success',
        'API 密钥': 'API Keys',
        '用于创建和管理 API 密钥。': 'Used to create and manage API keys.',
        '点击后可新增、查看和删除密钥。': 'Click to create, view and delete keys.',
        '创建并管理你的 API 密钥': 'Create and manage your API keys',
        '创建并管理你的 API Keys': 'Create and manage your API keys',
        '创建密钥': 'Create Key',
        '管理你的密钥以访问全部模型': 'Manage your keys to access all models',
        '管理你的密钥以访问All模型': 'Manage your keys to access all models',
        '名称': 'Name',
        '密钥': 'Key',
        '过期时间': 'Expiry Time',
        '过期Time': 'Expiry Time',
        ...
        '发布': 'Publish',
        '级联': 'Cascade',
        '用于查看推荐频道、搜索并进入已加入频道。': 'Used to view recommended channels, search, and enter joined channels.',
        '点击后进入订阅页面。': 'Click to open the subscribe page.',
        '用于创建和管理频道的发布入口。': 'Used to create and manage publishing channels.',
        '点击后进入发布页面。': 'Click to open the publish page.',
        '用于配置互动级联服务器地址。': 'Used to configure the interaction cascade server address.',
        '点击后进入服务器设置页面。': 'Click to open server settings.',
        '服务器设置': 'Server Settings',
        '设置互动级联服务器地址': 'Set the interaction cascade server address',
        '服务器地址': 'Server Address',
        '保存设置': 'Save Settings',
        '当前服务器': 'Current Server',
        '当前未配置服务器地址': 'No server address configured',
        '服务器设置已保存': 'Server settings saved',
        '保存服务器设置失败': 'Failed to save server settings',
        '加载服务器设置失败': 'Failed to load server settings',
        '服务器地址格式不正确': 'Invalid server address',
        'LinkMind 用户': 'LinkMind User',
        '请先登录后再使用频道功能': 'Please sign in before using channels.',
        '暂无频道介绍': 'No channel description',
        '未命名频道': 'Unnamed channel',
        '公开频道': 'Public channel',
        '私有频道': 'Private channel',
        '已启用': 'Enabled',
        '已停用': 'Disabled',
        '我创建的频道': 'Created by me',
        '没有匹配到推荐频道': 'No recommended channels matched',
        '已加入': 'Joined',
        '加入': 'Join',
        '当前没有匹配到已加入频道': 'No joined channels matched',
        '退出': 'Leave',
        '已退出': 'Left',
        '频道操作失败': 'Channel operation failed',
        '退出频道失败': 'Failed to leave channel',
        '推荐频道': 'Recommended Channels',
        '发现你可能感兴趣的频道': 'Discover channels you may be interested in',
        '我加入的频道': 'My Joined Channels',
        '查看已加入频道的最新动态': 'View the latest updates from joined channels',
        '最新信息': 'Latest Info',
        '加载频道失败': 'Failed to load channels',
        '没有匹配到可管理频道': 'No manageable channels matched',
        '停用': 'Disable',
        '状态切换失败': 'Failed to change status',
        '已删除': 'Deleted',
        '删除频道失败': 'Failed to delete channel',
        '频道管理': 'Channel Management',
        '创建你自己的频道': 'Create your own channel',
        '创建': 'Create',
        '创建频道': 'Create Channel',
        '填写频道信息后即可发起创建': 'Fill in channel details to create it',
        '管理列表': 'Management List',
        '可对现有频道进行停用或删除': 'Disable or delete existing channels',
        '频道名称': 'Channel Name',
        '请输入频道名称': 'Please enter a channel name',
        '频道介绍': 'Channel Description',
        '可选': 'Optional',
        '请输入频道介绍（可选）': 'Please enter a channel description (optional)',
        '创建频道失败': 'Failed to create channel',
        '加载管理频道失败': 'Failed to load managed channels',
        '频道模块初始化失败': 'Failed to initialize channel module',
        '监控': 'Monitor',
        '刷新间隔': 'Refresh interval',
        '秒': 'sec',
        '立即刷新': 'Refresh now',
        '已更新刷新间隔': 'Refresh interval updated',
        '频道实时消息': 'Real-time channel messages',
        ...
        '请求明细日志（前端基于已落库 Token 记录构建）': 'Request logs (built on persisted token records in frontend).',
        '按模型筛选（可选）': 'Filter by model (optional)',
        '日志中心（Generations / Jobs / Sessions）': 'Log center (Generations / Jobs / Sessions)',
        'Generations：模型调用请求日志': 'Generations: model request logs',
        'Jobs：按天聚合任务视图（由请求数据汇总）': 'Jobs: daily aggregated job view',
        'Sessions：按小时会话窗口聚合': 'Sessions: hourly session window aggregation',
        'Jobs：来自安全过滤监控记录（fence）': 'Jobs: from security fence monitor logs',
        'Sessions：来自当前会话上下文（CONVERSATION_CONTEXT）': 'Sessions: from current conversation context',
        '当前页会话': 'Current page session'
        ,
        '暂无进行中的任务': 'No jobs in progress',
        '任务页会显示当前正在执行的异步任务<br/>（例如由服务端返回的进行中任务）。': 'Jobs page shows async tasks currently in progress<br/>(for example, in-progress tasks returned by the server).',
        '动作': 'Action',
        '暂无会话数据': 'No session data',
        '你可以通过传入同一个 session_id 来聚合多次请求，<br/>用于跟踪完整会话与多步任务链路。': 'You can pass the same session_id to group multiple requests,<br/>for tracking full conversations and multi-step task chains.',
        'session_id 请求示例：': 'session_id request example:',
        '请求次数': 'Requests',
        '首次请求时间': 'First Request Time',
        ...
        'Landing 配置需要登录后获取用户 ID，请先登录': 'Landing requires your user ID. Please sign in first.',
        'OpenAICompatible 类型需要 name': 'OpenAICompatible requires a name',
        'OpenAICompatible 类型需要 model': 'OpenAICompatible requires a model',
        '必填，仅列表展示': 'Required; list label only',
        '名称为必填项': 'Name is required',
        '仅管理云端 Landing 密钥（本地密钥已关闭）': 'Only cloud Landing API keys are managed (local keys are disabled).',
        '复制': 'Copy',
        '复制失败': 'Copy failed',
        '暂无可复制的密钥': 'No key available to copy',
        '例如：工作用 Key': 'e.g. work key',
        '新增失败': 'Create failed',
        '请输入模型名称': 'Please enter model name',
        '确认删除该模型的 API Key 吗？': 'Delete this model\'s API key?',
        '管理模型的 API Key': 'Manage API keys for models',
        '新增配置': 'Add configuration',
        '提供商': 'Provider',
        'API 地址': 'API URL',
        'API Key 配置': 'API Key configuration',
        'api_address（可选）': 'API URL (optional)',
        '例如：custom2': 'e.g. custom2',
        '例如：gpt-4o-mini': 'e.g. gpt-4o-mini',
        '例如：https://xxx/v1/chat/completions': 'e.g. https://xxx/v1/chat/completions',
        '新增': 'Add',
        '保存修改': 'Save changes',
        '模型名称': 'Model name',
        '无需填写 API Key：密钥将由服务端自动生成，并写入配置文件 lagi.yml。': 'No API key needed: the server will generate it and write it to lagi.yml.',
        '请先登录': 'Please sign in first',
        '获取收款码失败': 'Failed to get payment QR code',
        '微信扫码支付': 'WeChat Pay (scan QR code)',
        '支付金额': 'Amount',
        ...
        '交易序列号': 'Transaction ID',
        '暂无交易记录': 'No transactions',
        '充值金额': 'Top-up amount',
        '应付总额': 'Total due',
        '确认充值': 'Confirm top-up',
        '充值成功后将新增交易记录，并更新当前余额。': 'After successful payment, a transaction will be added and your balance updated.',
        '共 ': 'Total ',
        ' 条': ' records',
        '模型': 'Model'
    };
